# 📍 LinkTrace PRO - Link Analytics & Visitor Location Tracking App

LinkTrace PRO adalah aplikasi *Link Shortener & Geolocation Analytics* modern yang mampu melacak dan mendeteksi lokasi presisi (GPS & IP Geolocation) pengakses link, mendeteksi sumber lalu lintas (*WhatsApp*, *Instagram*, *Facebook*, *Direct*, dll.), jenis perangkat/OS, serta peramban web (*browser*) secara real-time.

---

## ✨ Fitur Utama

- 🎯 **GPS Precision Location Capture**: Halaman perantara pintar (*smart landing page*) yang meminta izin lokasi GPS berpresisi tinggi (`navigator.geolocation`) dari HP/Komputer pengakses.
...
- 📑 **Full-Width Real-Time Visitor Logs**: Tabel log pengunjung lengkap dengan status koordinat, alamat IP, peramban, serta tombol tautan langsung ke **Google Maps**.
- 🔒 **Admin Authentication**: Dilengkapi dengan pengaman sesi login admin (Token-based Auth).
- 💾 **Persistent JSON Database**: Menyimpan seluruh data link dan riwayat pengunjung secara permanen di server (`data/links.json`).
